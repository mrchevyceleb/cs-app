// Dashboard Components
export { MetricsBar } from './MetricsBar'
export { Sidebar } from './Sidebar'
export { ConfidenceScore } from './ConfidenceScore'
export { TicketCard, TicketCardSkeleton } from './TicketCard'
export type { TicketWithCustomer } from './TicketCard'
export { TicketQueue } from './TicketQueue'
export { FilterBar, defaultFilters } from './FilterBar'
export type { FilterOptions } from './FilterBar'
export { GetNextTicketButton, GetNextTicketButtonCompact, GetNextTicketButtonIcon } from './GetNextTicketButton'

// Ticket Detail Components
export { TicketDetail } from './TicketDetail'
export { ChatBubble, ChatBubbleSkeleton } from './ChatBubble'
export { ChatInput } from './ChatInput'
export { TypingIndicator, TypingIndicatorCompact } from './TypingIndicator'
export { CustomerContext } from './CustomerContext'
export { TagManager } from './TagManager'

// File Attachments
export { FileUpload, FileChip } from './FileUpload'
export type { UploadedFile } from './FileUpload'
export { MessageAttachment, MessageAttachments } from './MessageAttachment'

// Analytics Components
export { CsatMetrics } from './CsatMetrics'
export { SlaMetrics } from './SlaMetrics'
export { ExportButton } from './ExportButton'

// Agent Performance Components
export { AgentPerformanceCard } from './AgentPerformanceCard'
export { AgentLeaderboard } from './AgentLeaderboard'
