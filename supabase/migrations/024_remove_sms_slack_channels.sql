...
-- 3. Backfill legacy sms/slack data in messages and tickets
UPDATE messages
SET source = 'email'
WHERE source IN ('sms', 'slack');

UPDATE tickets
SET source_channel = 'email'
WHERE source_channel IN ('sms', 'slack');

-- 4. Update check constraint on channel_config.channel
...
-- 5. Update check constraint on messages.source
...
-- 6. Update check constraint on tickets.source_channel
ALTER TABLE tickets
...
-- 7. Update check constraint on customers.preferred_channel
